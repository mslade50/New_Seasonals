"""Run the frozen gap-reversal v1 from explicit local research inputs."""
...
def _load_snapshot_provenance(
    ...
    *,
    ...
    if data_dir.resolve() != expected_bars:
        raise ValueError(f"data directory must be the snapshot bars directory: {expected_bars}")
...
def _validate_used_files(result, item_hashes: dict[str, str]) -> int:
    if not item_hashes:
        return 0
...
def _default_output_dir() -> Path:
    ...
    return ROOT / "artifacts" / "gap-reversal-v1" / stamp


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Research-only gap-reversal v1 runner. Reads local parquets and "
            "never downloads, uploads, stages, schedules, or promotes."
        )
    )
...
def main() -> int:
    args = parse_args()
    sector_hash = _validated_hash(
        args.sector_map, args.expected_sector_map_sha256, "sector map"
    )
    universe_hash = (
        _validated_hash(args.universe_file, args.expected_universe_sha256, "universe file")
        if args.universe_file is not None
        else None
    )
    snapshot, item_hashes = _load_snapshot_provenance(
    ...
    provenance = {
        **snapshot,
        ...
        "sector_map_file_sha256": sector_hash,
        "universe_file_path": str(args.universe_file.resolve()) if args.universe_file else None,
    ...
    result.source_provenance["n_loaded_files_validated_against_snapshot"] = (
        _validate_used_files(result, item_hashes)
    )
    output = write_gap_reversal_artifacts(result, args.output_dir or _default_output_dir())
    print(f"Research artifacts: {output}")
    print(
        f"Requested {len(result.requested_tickers)} | "
        f"evaluated {len(result.loaded_candidate_tickers)} | "
        f"candidates {len(result.signals)} | fills {len(result.trades)}"
    )
    print(
        result.primary_day_cluster_stats.loc[
            result.primary_day_cluster_stats["primary_cost_case"]
        ].to_string(index=False)
    )
    print("Research-only: no network, R2 mutation, production write, broker action, order, schedule, or promotion occurred.")
    return 0
