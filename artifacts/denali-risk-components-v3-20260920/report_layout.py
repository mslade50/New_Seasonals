"""Shared docx/markdown layout. Same visual grammar as the 17 September build;
only the footer date moves."""
...
FOOTER='DENALI  |  Internal  |  20 September 2026'
...
def build(name,title,pages,compact=False):
    ...
    for nm,sz in [('Normal',10 if compact else 10.5),('Title',21),('Heading 1',13)]:
        ...
        st.font.color.rgb=RGBColor.from_string('000000' if nm!='Heading 1' else '19384A')
    ...
    for n,page in enumerate(pages):
        # A standalone break paragraph can itself be pushed off a full page and
        # leave a blank sheet behind it, so the break rides on the first
        # paragraph of the new page instead.
        pending=bool(n)
        if n:md.append('\n---\n')
        for b in page:
            lead=None
            if b[0]=='h':lead=d.add_heading(b[1],1);md.extend(['## '+b[1],''])
                ...
                for row,values in [(t.rows[0],headers)]+[(t.add_row(),v) for v in rows]:
                    ...
                    for ci,(cell,text,w) in enumerate(zip(row.cells,values,widths)):
                        ...
                        p.paragraph_format.space_before=Pt(3);p.paragraph_format.space_after=Pt(5 if compact else 6)
                ...
                for cell in t.rows[0].cells:
                    sh=OxmlElement('w:shd');sh.set(qn('w:fill'),'E8EFF2');cell._tc.get_or_add_tcPr().append(sh)
