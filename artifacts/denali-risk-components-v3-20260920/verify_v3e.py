"""Text-level checks on the rendered v3e PDFs: no retired headings, no book leakage.

Every v3c check, plus the wording the move from 50 to 55 has to leave behind, and the
headings v3d and v3e each have to introduce.
"""
...
DOCS = {
    'Denali_Risk_Dial_Detailed_v3e': {'pages': 4, 'need': ['NYSE Net Highs', 'Why 55 matters', 'When the dial matters most']},
    'Denali_Risk_Dial_One_Page_v3e': {'pages': 1, 'need': []},
}
TITLE = 'Denali Risk Dial'
# Text that must appear somewhere in the document (not necessarily as a heading).
REQUIRED = {'Denali_Risk_Dial_Detailed_v3e': ['Why 55 matters', 'When the dial matters most', 'Below 55', '55 or above', 'near-high'],
            'Denali_Risk_Dial_One_Page_v3e': ['55 or above', 'Where it matters most']}
...
RETIRED = ['5 day dial', '21 day dial', '5-day dial', '21-day dial', 'pre fomc', 'pre-fomc',
           ...
           'why 50 matters', '50 or above', 'below 50', 'fifty is our']
...
for name, spec in DOCS.items():
    ...
    for need in REQUIRED.get(name, []):
        if need.lower() not in low:
            fail.append(f'{name}: missing required text "{need}"')
    for term in RETIRED:
        if term in low:
