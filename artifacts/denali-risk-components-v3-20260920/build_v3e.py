...
import csv, json
import report_layout_v3e as layout

ROOT = Path(__file__).resolve().parent
comp = {r['component']: r for r in
        csv.DictReader((ROOT / 'v3e_component_returns.csv').open(encoding='utf-8'))}
bands = list(csv.DictReader((ROOT / 'v3e_main_dial_ranges.csv').open(encoding='utf-8')))
near_bands = list(csv.DictReader((ROOT / 'v3e_near_high_ranges.csv').open(encoding='utf-8')))
...
near_band_rows = [[b['range'], f"{int(b['n']):,}", *row(b)] for b in near_bands]
near_band_rows.append(['All near-high dates', NEAR_N, *row(NEAR['baseline'])])
near_split_rows = [[f'Below {CUT}', f"{NEAR['below_cutoff']['n']:,}", *row(NEAR['below_cutoff'])],
                   [f'{CUT} or above', f"{NEAR['at_or_above_cutoff']['n']:,}",
                    *row(NEAR['at_or_above_cutoff'])]]
...
('h', 'When the dial matters most'),
('p', f"Most of the components can only fire while SPY is close to its trailing high. Five of the six price components did so on essentially every warning in this study, and the NYSE warning is defined to require the index within 3% of its high. That is deliberate: the dial is built to flag risk while the index still looks healthy, not to describe a decline already under way. About half the study, {NEAR_N} of the {N} dates, had SPY closing less than 2% below its trailing 252-session high. Those are the dates the dial is speaking about, and the same tables look sharper on them."),
('table', ['Main dial, near-high dates', 'Dates', *WINDOWS], near_band_rows, [2.8, .6, 1.3, 1.3, 1.3]),
('p', f'Grouped at the {CUT} line, the same near-high dates look like this:'),
('table', ['Main dial, near-high dates', 'Dates', *WINDOWS], near_split_rows, [2.8, .6, 1.3, 1.3, 1.3]),
('p', f"On these dates the {CUT} line separates positive average returns from negative ones at all three horizons, which the split across the whole study does not. The order of the bands is the same as before, including the same slight lift in the {CUT} to 65 band, and the readings above 65 are weaker here than in the full sample. A month-ahead loss followed {share(NEAR['loss21_share_below'])} of the near-high dates below {CUT}, {share(NEAR['loss21_share_at_or_above'])} of those at {CUT} or above, and {share(NEAR['loss21_share_80plus'])} of those at 80 and above."),
('p', f"The contrast on the other side is just as useful. On the {OFF['n']:,} dates where SPY closed 2% or more below its high, a high reading was not followed by weak returns: the {OFF['at_or_above_cutoff']['n']} of them at {CUT} or above averaged {', '.join(row(OFF['at_or_above_cutoff']))} over the three windows. A high reading there mostly describes a decline that has already happened, which is not what the dial is for."),
('small', note),
...
layout.build('Denali_Risk_Dial_Detailed_v3e', 'Denali Risk Dial', pages)
...
layout.build('Denali_Risk_Dial_One_Page_v3e', 'Denali Risk Dial', [[
('p', intro),
...
('table', ['Main dial', 'Dates', *WINDOWS], band_rows, [2.8, .6, 1.3, 1.3, 1.3]),
('p', f'**Reading it:** below 20 has been most supportive; 20 to 40 constructive; 40 to 55 calls for caution; {CUT} and above is the working warning threshold, set at the 85th percentile of readings, with all three windows negative from 65.'),
('small', f'**Where it matters most:** most components can only fire while SPY is near its trailing high, and on the {NEAR_N} dates closing less than 2% below it the threshold separates positive averages from negative ones at every horizon, {", ".join(row(NEAR["at_or_above_cutoff"]))} at {CUT} or above against {", ".join(row(NEAR["below_cutoff"]))} below the line, while more than 2% below the high a high reading was not followed by weak returns.'),
