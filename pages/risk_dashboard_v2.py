...
# Fragility scoring core moved to fragility_core.py (2026-07-16, A3).
...
# compute_fragility_timeseries also lives in fragility_core (A3 move).
...
def _cached_compute_signals(_spy_df, _closes, _sp500_closes, cache_key):
    ...
    # Shared scoring pipeline (fragility_core, A3). The page is the only
    # caller that refreshes rd2_fragility_ts.parquet — an explicit, vintage-
    # stamped write for its research/ML consumers, never a sizing input.
    bundle = compute_fragility_bundle(
        signals_ordered, regime_mult, price_ctx, spy_close,
        ts_write_path=os.path.join(DATA_DIR, "rd2_fragility_ts.parquet"))
