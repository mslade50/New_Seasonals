...
SECTOR_ETFS = [
    ...
    # --- Diversified megacaps (>20yr history, huge cap) ---
    ...
    "AMT",     # Real Estate / Towers
]

# Glossary: (description, IBKR tradeable?)
# "Y" = directly tradeable, "ETF" = trade via ETF proxy, "F" = futures, "N" = not tradeable
# All names here are large-cap US common stocks — directly tradeable ("Y").
TICKER_INFO = {
    # --- Dow Jones Industrial Average (30) ---
    "MMM": ("3M", "Y"), "AXP": ("American Express", "Y"),
    "AMGN": ("Amgen", "Y"), "AMZN": ("Amazon", "Y"),
    "AAPL": ("Apple", "Y"), "BA": ("Boeing", "Y"),
    "CAT": ("Caterpillar", "Y"), "CVX": ("Chevron", "Y"),
    "CSCO": ("Cisco", "Y"), "KO": ("Coca-Cola", "Y"),
    "DIS": ("Disney", "Y"), "GS": ("Goldman Sachs", "Y"),
    "HD": ("Home Depot", "Y"), "HON": ("Honeywell", "Y"),
    "IBM": ("IBM", "Y"), "JNJ": ("Johnson & Johnson", "Y"),
    "JPM": ("JPMorgan Chase", "Y"), "MCD": ("McDonald's", "Y"),
    "MRK": ("Merck", "Y"), "MSFT": ("Microsoft", "Y"),
    "NKE": ("Nike", "Y"), "NVDA": ("Nvidia", "Y"),
    "PG": ("Procter & Gamble", "Y"), "CRM": ("Salesforce", "Y"),
    "SHW": ("Sherwin-Williams", "Y"), "TRV": ("Travelers", "Y"),
    "UNH": ("UnitedHealth", "Y"), "VZ": ("Verizon", "Y"),
    "V": ("Visa", "Y"), "WMT": ("Walmart", "Y"),
    ...
    "NFLX": ("Netflix", "Y"), "CMCSA": ("Comcast", "Y"),
    "T": ("AT&T", "Y"), "NEE": ("NextEra Energy", "Y"),
    "UNP": ("Union Pacific", "Y"), "MA": ("Mastercard", "Y"),
    "BAC": ("Bank of America", "Y"), "AMT": ("American Tower", "Y"),
}
...
def main():
    ...
    # 1. TABLE SECTION
    with st.spinner("Loading Equity Metrics..."):
