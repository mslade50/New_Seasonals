...
# Set the cycle start year based on the user's selection
if cycle_label == "Election":
    cycle_start = 1952
elif cycle_label == "Pre-Election":
    cycle_start = 1951
elif cycle_label == "Post-Election":
    cycle_start = 1953
elif cycle_label == "Midterm":
    cycle_start = 1950

st.write(f"Cycle type selected: {cycle_label} (Start year: {cycle_start})")
...
if st.button('Plot'):
    try:
        seasonals_chart(stock, cycle_start, cycle_label)
    except Exception as e:
        st.error(f'Error retrieving data. Please check the ticker and try again. Details: {e}')
...
# 	average_correlation = f"{average_correlation:.2f}"
	...
	annotations = [
	    ...
	    create_annotation(0.55, -0.22, f"Total Avg: {all_avg}", text_color(all_avg)),
