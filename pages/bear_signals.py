...
file_names = ["bear_sigs.txt", "bear_filter.txt"]
...
for file_name in file_names:
    ...
    if response.status_code == 200:
        ...
        if file_name == "bear_sigs.txt":
            for ticker in tickers:
                megas_list.append(ticker.strip("'"))
        elif file_name == "bear_filter.txt":
            for ticker in tickers:
                bear_filter.append(ticker.strip("'"))

# Convert megas_list and bear_filter to sets to easily find intersection (common elements)
megas_set = set(megas_list)
bear_filter_set = set(bear_filter)

# Intersection of megas_set and bear_filter_set
intersection_list = list(megas_set & bear_filter_set)
