...
# -----------------------------------------------------------------------------
# NEW: Mark-to-Market Equity Curve Calculation
# -----------------------------------------------------------------------------
def calculate_mark_to_market_curve(sig_df, master_dict):
    """
    Calculates a daily Equity Curve based on Daily Mark-to-Market (MtM) PnL.
    Iterates day by day, sums realized PnL for closed trades AND 
    unrealized PnL for open trades.
    """
    if sig_df.empty:
        return pd.DataFrame(columns=['Equity', 'Daily PnL'])
    
    # 1. Define the Date Range for the Curve
    min_date = sig_df['Date'].min()
    # Use max Exit Date or today, whichever is later (to show current open exposure)
    ...
    for idx, trade in sig_df.iterrows():
        ...
        if t_df is None or t_df.empty:
            # Fallback: Just recognize PnL on exit date if no daily data
            daily_pnl[exit_date] += trade['PnL']
            continue
            
        # Slice relevant days for this trade
        # Start from day after entry (first day of PnL change) up to Exit Date
        trade_dates = all_dates[(all_dates > entry_date) & (all_dates <= exit_date)]
        
        # We need the daily price changes
        # Reindex t_df to these dates to handle missing days (holidays)
        # We need the price on entry_date as the starting point
        ...
        price_diffs = relevant_prices.diff().dropna() # This gives Change from Prev Day
        
        # Adjust for Short Selling
        if action == "SELL SHORT":
            price_diffs = -price_diffs
            
        # Calculate Daily Dollar PnL
        daily_dollar_pnl = price_diffs * shares
        
        # Add to the master Daily PnL Series
        # We must align indices carefully
    ...
    if run_btn:
        ...
        if all_signals:
            ...
            with col1:
                ...
                # --- NEW MARK-TO-MARKET CURVE LOGIC ---
