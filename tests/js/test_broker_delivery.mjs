...
function make(env={STATUS_TOKEN:'fixture',AGENT_TOKEN:'agent-fixture'}) {
  const memory=new Map();let calls=0, fail=true;
  const socket={deserializeAttachment:()=>({}),send:()=>{calls++;if(fail)throw Error('socket closed');}};
  const ctx={getWebSockets:()=>[socket],storage:{
  ...
  return {broker:new c.Broker(ctx,env),memory,calls:()=>calls,succeed:()=>{fail=false;}};
}
const failures=[];
...
await test('fill retention does not discard 501st or 1001st execution',async()=>{
  ...
  const fills=Array.from({length:1101},(_,i)=>({exec_id:`execution-${i}.01`,time:new Date(now).toISOString(),shares:1,price:100}));
  await x.broker._mergeFills({at:now,accounts:[{key:'primary',fills_complete:true,fills}]});
  ...
  await x.broker._mergeFills({at:now,accounts:[{key:'primary',fills_complete:true,fills:[{...fills[0],exec_id:'execution-0.02',shares:2}]}]});
...
await test('failed and legacy-capped sources never claim complete',async()=>{
  ...
  await x.broker._mergeFills({at:Date.now(),accounts:[{key:'primary',fills_complete:true,fills:[{exec_id:'new.01',time:new Date().toISOString()}]}]});
