...
dip-buyers 0.25x at 63d-MA10 >= 50; OVS fully exempt; OLV bandless again —
a 0.5x-at->=65 appetite band ran for one session 2026-08-24 -> 25 and was
retired in favour of a manual hedge), daily_scan applies them live via
frag_band_mult, strat_backtester replays them point-in-time via
frag_band_mult_at. The retired book-wide ramp (1.25x boost / 0.10x floor)
must stay dead.
"""
import os
import sys
...
def test_config_carries_exactly_the_family_band_entries():
    ...
    # OLV: a 0.5x-at->=65 appetite band ran 2026-08-24 -> 25 and was retired
    # (manual one-off hedge instead) — it must stay bandless and P/C-less.
    ...
    assert not _exec_of(OLV).get("frag_risk_bands")
    assert not _exec_of(OLV).get("pc_fear_bands")
...
def test_bandless_strategy_untouched_at_any_score():
    for name in ("St OS Sznl", OLV):
        ex = _exec_of(name)
        assert not ex.get("frag_risk_bands")
        for score in (0, 30, 55, 65, 90, None):
            assert frag_band_mult(ex, score) == 1.0
